...
export default function Category({ name, all }) {
  return (
    <div className="my-[12px] shadow-sm">
      <CategoryHeader name={name} all={all} />
    </div>
  );
}
...
function CategoryHeader({ name, all }) {
  return (
    <div className="flex justify-between items-center py-[8px] px-[12px]">
      <Link to={`/category/${name}`} className="flex items-center gap-[4px]">
        <svg
        ...
        <span className="font-Lato text-[#0095FF]">{all && "شاهد الكل"}</span>
      </Link>
